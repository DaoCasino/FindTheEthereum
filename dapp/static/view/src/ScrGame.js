...
var ScrGame = function(){
	...
	var _tfBalance, _tfBet, _tfAddress, _tfTime, _tfBlockchain, _tfWarning;
	...
	// windows
	var _wndDeposit, _wndBet, _wndWarning, _wndInfo, _wndWin, _wndHistory, _wndInstruction;
	// boolean
	...
	_self.showWndHistory = function() {
		...
		if(options_arcade){
			...
			_wndHistory.show(DApp.logic.history)
		}
		_timeCloseWnd = 0;
		...
		DApp.connect(objConnect, function (connected, info) {
			...
			if(connected){
				...
				if (info.channel) {
					...
					_endBlock = Number(info.channel.endBlock) + 3;
					_depositBankroll = DCLib.Utils.dec2bet(Number(info.channel.bankrollerBalance));
				}
				_wndWarning.visible = false;
				_balanceSession = deposit;
				_depositPlayer = deposit; 
	...
	_self.clickBox = function(box) {
		...
		if (options_arcade) {
			var session = _logic.session();
			var betGame = DCLib.Utils.bet2dec(_betGame);
			var gameData = { type: 'uint', value: [betGame, box.id] };
			result = _logic.clickBox(session, gameData);
			_self.showResult(result, box);
		} else {
			const rndHash = DCLib.randomHash({ bet: _betGame, gamedata: [_idBox] });
			DApp.Game(_betGame, _idBox, rndHash)
	...
	// RESULT
	_self.showResult = function(response, box){
		var result = response.local.result;
		_balanceSession = result.balance;
		if(options_debug){
			//_balanceSession = _depositPlayer + App.logic.payChannel.getProfit()
		}
		
		if (response.local.result.profit != response.bankroller.result.profit){
			// TODO: Dispute
			_self.showError(getText("Conflict closeGame"));
		}

		_self.refreshBalance();
		_balanceGame = DCLib.Utils.dec2bet(result.profit);
		_self.saveGame();
		...
		var win = (result.valPlayer == result.randomNum);
		...
		var it = new ItemResult(win, box.x, box.y-200)
		face_mc.addChild(it)
		...
		createjs.Tween.get({}).to({},1000)
			.call(function(){
				for (var i = 0; i < _arBoxes.length; i++) {
					var item_mc = _arBoxes[i];
					if (item_mc.id == result.randomNum){
